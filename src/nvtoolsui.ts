...
export default class NVToolsUI extends Plugin {
	...
	private _images: Array<ModelItem> = [];
	private _imagesElement: Array<ExternalImagesElementStatus> = [];
     ...
     * @returns
     */
	private _listExternalImages(): Array<ElementView> {
		const locale = this.editor.locale;
		// const t = locale.t;

		this._imagesElement = [];
		const images: Array<ElementView> = [];
	...
	private async _saveExternalImageRun(): Promise<void> {
		const editor = this.editor;
		if ( !this._saveExternalImageIsRun ) {
			return;
		}
     ...
     */
	public showDialogSaveExternalImage(): void {
		const editor = this.editor;
		const dialog = editor.plugins.get( 'Dialog' );
		const t = editor.locale.t;

		if ( !this._formSaveExternalImageView ) {
			this._formSaveExternalImageView = new ( CssTransitionDisablerMixin( NVToolsSaveExternalImageFormView ) )(
				getFormSaveImgValidators( this.editor ), this.editor
			);
		}
		...
		const actionButtons: Array<DialogActionButtonDefinition> = [];
     ...
     * @param path
     */
	public setExternalImagePath( path: string ): void {
		if ( this._formSaveExternalImageView ) {
